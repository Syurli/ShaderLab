...
vec3 safeNorm(vec3 v){return v/max(length(v),0.0001);}
vec3 rotateAxis(vec3 v,vec3 axis,float a){float s=sin(a);float c=cos(a);return v*c+cross(axis,v)*s+axis*dot(axis,v)*(1.0-c);}
vec3 hsv2rgb(vec3 c){vec3 k=vec3(0.0,0.6666667,0.3333333);vec3 p=abs(fract(vec3(c.x)+k)*6.0-vec3(3.0));return c.z*mix(vec3(1.0),clamp(p-vec3(1.0),vec3(0.0),vec3(1.0)),c.y);}
...
vec3 orbitNormal(float lane){
  if(lane<0.5) return safeNorm(vec3(0.22,0.94,0.26));
  if(lane<1.5) return safeNorm(vec3(-0.61,0.63,0.48));
  if(lane<2.5) return safeNorm(vec3(0.72,0.38,-0.58));
  return safeNorm(vec3(-0.32,0.76,-0.56));
}

void orbitFrame(float eventId,float lane,out vec3 radial,out vec3 tangent){
  vec3 normal=orbitNormal(lane);
  vec3 helper=abs(normal.y)<0.92?vec3(0.0,1.0,0.0):vec3(1.0,0.0,0.0);
  vec3 a=safeNorm(cross(helper,normal));
  vec3 b=safeNorm(cross(normal,a));
  float phase=eventId*GOLDEN+lane*1.117+hash11(eventId*1.91+lane*4.7)*0.84;
  radial=a*cos(phase)+b*sin(phase);
  tangent=-a*sin(phase)+b*cos(phase);
  vec3 spinAxis=safeNorm(vec3(0.18,1.0,0.07));
  float orbitAngle=uTime*0.18*uOrbitRotationSpeed;
  radial=safeNorm(rotateAxis(radial,spinAxis,orbitAngle));
  tangent=rotateAxis(tangent,spinAxis,orbitAngle);
  tangent=safeNorm(tangent-radial*dot(tangent,radial));
}
...
void main(){
  ...
  float shellVisibility=smoothstep(coverageThreshold-0.075,coverageThreshold+0.075,shellField(materialDir));
  ...
  for(int i=0;i<4;++i){
    ...
    for(int h=0;h<3;++h){
      ...
      float influence=(1.0-smoothstep(uInfluenceRadius*0.42,uInfluenceRadius,localRadius))*smoothstep(0.52,0.99,facing);

      float randomBend=(hash11(eventIndex*13.7+fi*2.1)-0.5)*uShapeRandomness;
      float centerLine=sin(along*mix(14.0,25.0,sc)+eventIndex*1.7+fi)*uRibbonWidth*(0.35+0.75*uShapeRandomness)+randomBend*uRibbonWidth*0.70;
      ...
      float filamentNoise=clamp(0.56+0.27*sin(along*56.0+sa*4.0+eventIndex)+0.17*sin(along*91.0-sc*8.0-fi*2.3),0.0,1.0);
      float density01=clamp((uEjectionDensity-0.5)/3.5,0.0,1.0);
      float threshold=mix(0.64,0.015,density01);
      float densityMask=smoothstep(threshold,min(threshold+mix(0.24,0.15,density01),0.98),filamentNoise);
      ...
      vec3 gatherOffset=tangentB*(-(across-centerLine)*funnel*gather)+tangentA*(-along*0.22*gather);
      ...
      eruptionOffset+=dir*(residual*uSurfaceWave*0.72*settleMask)+tangentA*(residualSide*uSurfaceWave*0.24*settleMask);
      ...
      surfaceResponse+=enabled*step(0.0,eventAge)*(ring*reached*rangeMask*uSurfaceWave+localKick*uSurfaceWave*0.30);
    }
  }
  ...
  float hue=fract(uProminenceHueOffset+pow(colorHeight,0.78)*uProminenceHueSpan);
  vec3 rainbow=hsv2rgb(vec3(hue,clamp(uProminenceSaturation,0.0,1.0),uProminenceBrightness));
  float rainbowMix=smoothstep(0.018,0.16,colorHeight)*eruptionVisual;
  vec3 shellColor=uShellColor*uShellBrightness;
  vColor=mix(shellColor,rainbow,rainbowMix);

  float tw=0.5+0.5*sin(uTime*mix(2.0,6.4,sb)+sa*61.0);
  vSpark=pow(tw,9.0)*(0.30+0.70*sc);
  vAlpha=shellVisibility*mix(0.16,0.76,rainbowMix)*(0.78+0.22*sb)*mix(1.0,0.94,settlingVisual);
  vSeed=sa;
  float size=mix(0.0078,0.0115,sb)*uParticleSize*mix(1.0,1.30,rainbowMix)*(1.0+vSpark*0.28);
