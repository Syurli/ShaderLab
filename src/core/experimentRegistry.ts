...
export const experiments: ExperimentDefinition[] = [
  ...
  defineExperiment({
    ...
    description: {
      zh: 'Raw WebGPU + WGSL 的二维解算/三维体积混合实验：平静核心由连续世界空间 Spiral/Ridged Noise 形成细腻蠕动的烟雾球；点击或音频节拍触发无极坐标接缝的分形爆裂，96×96 二维场通过三平面采样抬升到世界空间三维密度，生成粗枝杈、褶皱薄片和烟雾尾迹。相机只改变真实 ro/rd，不再参与密度投影，因此轨道观察能看到不同侧面。',
      en: 'A Raw WebGPU + WGSL 2D-solver/3D-volume hybrid. The idle core uses continuous world-space spiral/ridged noise for a finely rolling smoke sphere. Clicks or audio beats trigger seam-free fractal bursts; a 96×96 2D field is lifted into world-space 3D density with tri-planar sampling to form broad branches, folded sheets, and smoke trails. The orbit camera now changes only the true ray origin/direction and no longer participates in density projection, so different viewpoints reveal genuinely different sides.',
    },
    category: { zh: '体积', en: 'Volume' },
    tags: ['WebGPU', 'WGSL', 'Volume', '2D Fluid', 'Spiral Noise', 'Fractal', 'Burst', 'Audio Reactive', 'Raymarch'],
